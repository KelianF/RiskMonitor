...
def Updatedb(Ticker, Start = "20000101", End = pd.Timestamp.today().strftime("%Y%m%d")):
    ...
    if Ticker + ".csv" in os.listdir(r"\\10.155.31.53\Share\Kelian\DATA\COMMO"):   # Folder Location
        #print("Got From DataBase")
        Pointer = pd.read_csv(r"\\10.155.31.53\Share\Kelian\DATA\COMMO\\" + Ticker + ".csv", index_col=0)
        if str(Pointer.index.max()) < End:
            ...
            Pointer.to_csv(r"\\10.155.31.53\Share\Kelian\DATA\COMMO\\" + Ticker + ".csv", index=True)
